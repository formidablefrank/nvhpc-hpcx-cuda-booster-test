...
bootstrap_env_dir="${repo_root}/env-nvhpc-26.3-hpcx-2.20-bootstrap"
...
nvhpc_spec="nvhpc@26.3 %gcc@12.2.0 ~mpi +blas +lapack default_cuda=12.2"
binutils_spec="binutils@2.42 %gcc@12.2.0"

spack -e "${bootstrap_env_dir}" concretize -f
spack -e "${bootstrap_env_dir}" install \
  --fail-fast \
  --no-checksum \
  --keep-prefix \
  --keep-stage \
  --show-log-on-error
nvhpc_prefix="$(spack -e "${bootstrap_env_dir}" location -i "${nvhpc_spec}")"
binutils_prefix="$(spack -e "${bootstrap_env_dir}" location -i "${binutils_spec}")"
